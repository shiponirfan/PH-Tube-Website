...
const handlerShowCategories = async (categories_id) => {
    ...
    data.data.forEach(categories =>{
        ...
        <div class="${categories.others?.posted_date? 'block' : 'hidden'} absolute bottom-3 right-3 bg-[#171717] rounded-lg">
            <h3 class="text-white p-2 ">${posted_date}</h3>
        </div>
                <img class="rounded-lg h-52 w-full" src="${categories.thumbnail}" alt="image" />
            </figure>
        <div class="flex mt-5">
